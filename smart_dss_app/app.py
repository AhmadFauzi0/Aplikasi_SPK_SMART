...
import io
import matplotlib
matplotlib.use('Agg')  # Mencegah error GUI thread pada web server
...
app = Flask(__name__)
app.secret_key = "secret_programmer_senior_key"
init_db()
engine = SMARTEngine()
...
def update_data(id):
    # Mengambil input dinamis dari tabel form web
...
def upload_file(id):
    ...
    # Otomatisasi mapping data terstruktur dari file unggahan
    ...
    with SessionLocal() as db:
        ...
        current_data["types"] = ["benefit"] * len(criteria)  # Default type
        current_data["weights"] = [100.0 / len(criteria)] * len(criteria)  # Default weight
        current_data["alternatives"] = alts
        current_data["performance"] = performance
        proj.data_json = current_data
        db.commit()
        
    flash("Data Berhasil Diunggah dari File Eksternal!", "success")
    return redirect(url_for('view_project', id=id))
...
def calculate_project(id):
    ...
    # Otomatisasi pembuatan visualisasi analisis grafik sensitivitas
    target_idx = 0  # Default menguji kriteria pertama
    ...
    # Konversi grafik ke base64 agar bisa ditampilkan langsung di HTML tanpa simpan file fisik
    import base64
...
def export_excel(id):
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        df.to_excel(writer, sheet_name='Matriks Keputusan')
    output.seek(0)
